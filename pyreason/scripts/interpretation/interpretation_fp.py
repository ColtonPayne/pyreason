...
# Mapping from node to world
node_to_world_type = numba.types.DictType(node_type, world.world_type)
edge_to_world_type = numba.types.DictType(edge_type, world.world_type)
...
class Interpretation:
	...
	def __init__(self, graph, ipl, annotation_functions, reverse_graph, atom_trace, save_graph_attributes_to_rule_trace, persistent, inconsistency_check, store_interpretation_changes, update_mode, allow_ground_rules):
		...
		for n in self.graph.nodes():
			neighbor_list = numba.typed.List.empty_list(node_type)
			[neighbor_list.append(neigh) for neigh in self.graph.neighbors(n)]
			self.neighbors[n] = neighbor_list
	...
	def _init_interpretations_node(nodes, specific_labels):
		interpretations = numba.typed.Dict.empty(key_type=numba.types.int64, value_type=node_to_world_type)
		predicate_map = numba.typed.Dict.empty(key_type=label.label_type, value_type=list_of_nodes)

		# Initialize for time zero
		interpretations[0] = numba.typed.Dict.empty(key_type=node_type, value_type=world.world_type)
		...
		# Specific labels
		for label_key, ns in specific_labels.items():
			predicate_map[label_key] = numba.typed.List(ns)
			# for n in ns:
			# 	interpretations[n].world[label_key] = interval.closed(0.0, 1.0)
			# 	num_ga[0] += 1
	...
	def _init_interpretations_edge(edges, specific_labels):
		interpretations = numba.typed.Dict.empty(key_type=numba.types.int64, value_type=edge_to_world_type)
		predicate_map = numba.typed.Dict.empty(key_type=label.label_type, value_type=list_of_edges)

		# Initialize for time zero
		interpretations[0] = numba.typed.Dict.empty(key_type=edge_type, value_type=world.world_type)
		...
		# Specific labels
		for label_key, es in specific_labels.items():
			predicate_map[label_key] = numba.typed.List(es)
			# for e in es:
			# 	interpretations[e].world[label_key] = interval.closed(0.0, 1.0)
			# 	num_ga[0] += 1
	...
	def reason(interpretations_node, interpretations_edge, predicate_map_node, predicate_map_edge, tmax, prev_reasoning_data, rules, nodes, edges, neighbors, reverse_neighbors, rules_to_be_applied_node, rules_to_be_applied_edge, edges_to_be_added_node_rule, edges_to_be_added_edge_rule, rules_to_be_applied_node_trace, rules_to_be_applied_edge_trace, facts_to_be_applied_node, facts_to_be_applied_edge, facts_to_be_applied_node_trace, facts_to_be_applied_edge_trace, ipl, rule_trace_node, rule_trace_edge, rule_trace_node_atoms, rule_trace_edge_atoms, reverse_graph, atom_trace, save_graph_attributes_to_rule_trace, persistent, inconsistency_check, store_interpretation_changes, update_mode, allow_ground_rules, max_facts_time, annotation_functions, convergence_mode, convergence_delta, verbose, again):
		...
		while fp_loop:
			...
			while timestep_loop:
				...
				# Copy static atoms to new interpretation timestep
				# if persistent, then copy everything
				# If fp operation > 0 then do not copy unless it is empty. Prevents overriding values
				# from previous fp operation
				# Add a new interpretation for the new timestep
				# Nodes
				...
				if t > 0 and persistent:
					last_t_interp = interpretations_node[t-1]
					for n in last_t_interp:
						...
						new_w = interpretations_node[t][n].world
						for label_key in w:
							# Only copy if this is the first fp operation (fp_cnt == 0) or if the label doesn't exist
							if fp_cnt == 0 or label_key not in new_w:
								new_w[label_key] = w[label_key].copy()

				# If not persistent then copy only what is static
					...
					for n in last_t_interp:
						...
						new_w = interpretations_node[t][n].world
						for label_key in w:
							if w[label_key].is_static():
								# Only copy if this is the first fp operation (fp_cnt == 0) or if the label doesn't exist
								if fp_cnt == 0 or label_key not in new_w:
									print("Overwriting static label", label_key, "for node", n, "at time", t)
									new_w[label_key] = w[label_key].copy()

				# Edges
				# Only create new interpretation if it doesn't exist or if this is the first fp operation
				if t not in interpretations_edge or fp_cnt == 0:
					interpretations_edge[t] = numba.typed.Dict.empty(key_type=edge_type, value_type=world.world_type)
				
				if t > 0 and persistent:
					last_t_interp = interpretations_edge[t-1]
					for e in last_t_interp:
						# Add edge to new interpretation only if it doesn't exist
						if e not in interpretations_edge[t]:
							interpretations_edge[t][e] = world.World(numba.typed.List.empty_list(label.label_type))

						w = last_t_interp[e].world
						new_w = interpretations_edge[t][e].world
						for label_key in w:
							# Only copy if this is the first fp operation (fp_cnt == 0) or if the label doesn't exist
							if fp_cnt == 0 or label_key not in new_w:
								new_w[label_key] = w[label_key].copy()
				...
				for i in range(len(facts_to_be_applied_node)):
					if facts_to_be_applied_node[i][0] == t:
						comp, label_key, bnd, static, graph_attribute = facts_to_be_applied_node[i][1], facts_to_be_applied_node[i][2], facts_to_be_applied_node[i][3], facts_to_be_applied_node[i][4], facts_to_be_applied_node[i][5]
						# If the component is not in the graph, add it
						if comp not in nodes_set:
							nodes_set.add(comp)
							_add_node(comp, neighbors, reverse_neighbors, nodes, interpretations_node[t])
						elif comp not in interpretations_node[t]:
							_add_node_to_interpretation(comp, interpretations_node[t])

						print("Applying fact for node:", comp, label_key, bnd, static, graph_attribute, "at", t, "fp", fp_cnt)
						...
						if label_key in interpretations_node[t][comp].world and interpretations_node[t][comp].world[label_key].is_static():
							...
							if (save_graph_attributes_to_rule_trace or not graph_attribute) and store_interpretation_changes:
								rule_trace_node.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, label_key, bnd))
								if atom_trace:
								...
								for p1, p2 in ipl:
									if p1==label_key:
										rule_trace_node.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, p2, interpretations_node[t][comp].world[p2]))
										if atom_trace:
											_update_rule_trace(rule_trace_node_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), interpretations_node[t][comp].world[p2], facts_to_be_applied_node_trace[i])
									elif p2==label_key:
										rule_trace_node.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, p1, interpretations_node[t][comp].world[p1]))
										if atom_trace:
											_update_rule_trace(rule_trace_node_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), interpretations_node[t][comp].world[p1], facts_to_be_applied_node_trace[i])
	
						else:
							# Check for inconsistencies (multiple facts)
							if check_consistent_node(interpretations_node[t], comp, (label_key, bnd)):
								print("should be here")
								mode = 'graph-attribute-fact' if graph_attribute else 'fact'
								override = True if update_mode == 'override' else False
								u, changes = _update_node(interpretations_node[t], predicate_map_node, comp, (label_key, bnd), ipl, rule_trace_node, fp_cnt, t, static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_node_trace, i, facts_to_be_applied_node_trace, rule_trace_node_atoms, store_interpretation_changes, mode=mode, override=override)
							...
							# Resolve inconsistency if necessary otherwise override bounds
								...
								if inconsistency_check:
									resolve_inconsistency_node(interpretations_node[t], comp, (label_key, bnd), ipl, t, fp_cnt, i, atom_trace, rule_trace_node, rule_trace_node_atoms, rules_to_be_applied_node_trace, facts_to_be_applied_node_trace, store_interpretation_changes, mode=mode)
								else:
									u, changes = _update_node(interpretations_node[t], predicate_map_node, comp, (label_key, bnd), ipl, rule_trace_node, fp_cnt, t, static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_node_trace, i, facts_to_be_applied_node_trace, rule_trace_node_atoms, store_interpretation_changes, mode=mode, override=True)
						...
						if static:
							facts_to_be_applied_node_new.append((numba.types.uint16(facts_to_be_applied_node[i][0]+1), comp, label_key, bnd, static, graph_attribute))
							if atom_trace:
				...
				for i in range(len(facts_to_be_applied_edge)):
					if facts_to_be_applied_edge[i][0] == t:
						comp, label_key, bnd, static, graph_attribute = facts_to_be_applied_edge[i][1], facts_to_be_applied_edge[i][2], facts_to_be_applied_edge[i][3], facts_to_be_applied_edge[i][4], facts_to_be_applied_edge[i][5]
						# If the component is not in the graph, add it
						if comp not in edges_set:
							_add_edge(comp[0], comp[1], neighbors, reverse_neighbors, nodes, edges, label.Label(''), interpretations_node[t], interpretations_edge[t], predicate_map_edge, t)
							edges_set.add(comp)
						elif comp not in interpretations_edge[t]:
							_add_edge_to_interpretation(comp, interpretations_edge[t])
	
						# Check if bnd is static. Then no need to update, just add to rule trace, check if graph attribute, and add ipl complement to rule trace as well
						if label_key in interpretations_edge[t][comp].world and interpretations_edge[t][comp].world[label_key].is_static():
							# Inverse of this is: if not save_graph_attributes_to_rule_trace and graph_attribute
							if (save_graph_attributes_to_rule_trace or not graph_attribute) and store_interpretation_changes:
								rule_trace_edge.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, label_key, interpretations_edge[t][comp].world[label_key]))
								if atom_trace:
								...
								for p1, p2 in ipl:
									if p1 == label_key:
										rule_trace_edge.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, p2, interpretations_edge[t][comp].world[p2]))
										if atom_trace:
											_update_rule_trace(rule_trace_edge_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), interpretations_edge[t][comp].world[p2], facts_to_be_applied_edge_trace[i])
									elif p2 == label_key:
										rule_trace_edge.append((numba.types.uint16(t), numba.types.uint16(fp_cnt), comp, p1, interpretations_edge[t][comp].world[p1]))
										if atom_trace:
											_update_rule_trace(rule_trace_edge_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), interpretations_edge[t][comp].world[p1], facts_to_be_applied_edge_trace[i])
						else:
							# Check for inconsistencies
							if check_consistent_edge(interpretations_edge[t], comp, (label_key, bnd)):
								mode = 'graph-attribute-fact' if graph_attribute else 'fact'
								override = True if update_mode == 'override' else False
								u, changes = _update_edge(interpretations_edge[t], predicate_map_edge, comp, (label_key, bnd), ipl, rule_trace_edge, fp_cnt, t, static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_edge_trace, i, facts_to_be_applied_edge_trace, rule_trace_edge_atoms, store_interpretation_changes, mode=mode, override=override)
							...
							# Resolve inconsistency
								...
								mode = 'graph-attribute-fact' if graph_attribute else 'fact'
								if inconsistency_check:
									resolve_inconsistency_edge(interpretations_edge[t], comp, (label_key, bnd), ipl, t, fp_cnt, i, atom_trace, rule_trace_edge, rule_trace_edge_atoms, rules_to_be_applied_edge_trace, facts_to_be_applied_edge_trace, store_interpretation_changes, mode=mode)
								else:
						...
						if static:
							facts_to_be_applied_edge_new.append((numba.types.uint16(facts_to_be_applied_edge[i][0]+1), comp, label_key, bnd, static, graph_attribute))
							if atom_trace:
			...
			for idx, i in enumerate(rules_to_be_applied_node):
				t, comp, label_key, bnd, set_static = i[0], i[1], i[2], i[3], i[4]
				...
				if check_consistent_node(interpretations_node[t], comp, (label_key, bnd)):
					override = True if update_mode == 'override' else False
					u, changes = _update_node(interpretations_node[t], predicate_map_node, comp, (label_key, bnd), ipl, rule_trace_node, fp_cnt, t, set_static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_node_trace, idx, facts_to_be_applied_node_trace, rule_trace_node_atoms, store_interpretation_changes, mode='rule', override=override)
				...
				# Resolve inconsistency
					...
					if inconsistency_check:
						resolve_inconsistency_node(interpretations_node[t], comp, (label_key, bnd), ipl, t, fp_cnt, idx, atom_trace, rule_trace_node, rule_trace_node_atoms, rules_to_be_applied_node_trace, facts_to_be_applied_node_trace, store_interpretation_changes, mode='rule')
					else:
						u, changes = _update_node(interpretations_node[t], predicate_map_node, comp, (label_key, bnd), ipl, rule_trace_node, fp_cnt, t, set_static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_node_trace, idx, facts_to_be_applied_node_trace, rule_trace_node_atoms, store_interpretation_changes, mode='rule', override=True)
				...
				print(t, comp, label_key, bnd, update)
				print("interp change", interpretations_node[t][comp].world[label_key])
			...
			for idx, i in enumerate(rules_to_be_applied_edge):
				...
				if edge_l.value != '':
					for e in edges_added:
						if interpretations_edge[t][e].world[edge_l].is_static():
							continue
						if check_consistent_edge(interpretations_edge[t], e, (edge_l, bnd)):
							override = True if update_mode == 'override' else False
						...
						# Resolve inconsistency
							...
							if inconsistency_check:
								resolve_inconsistency_edge(interpretations_edge[t], e, (edge_l, bnd), ipl, t, fp_cnt, idx, atom_trace, rule_trace_edge, rule_trace_edge_atoms, rules_to_be_applied_edge_trace, facts_to_be_applied_edge_trace, store_interpretation_changes, mode='rule')
							else:
					...
					# if edge doesn't exist in interpretation, add it
					if comp not in interpretations_edge[t]:
						_add_edge_to_interpretation(comp, interpretations_edge[t])

					# Check for inconsistencies
					if check_consistent_edge(interpretations_edge[t], comp, (label_key, bnd)):
						override = True if update_mode == 'override' else False
						u, changes = _update_edge(interpretations_edge[t], predicate_map_edge, comp, (label_key, bnd), ipl, rule_trace_edge, fp_cnt, t, set_static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_edge_trace, idx, facts_to_be_applied_edge_trace, rule_trace_edge_atoms, store_interpretation_changes, mode='rule', override=override)
					...
					# Resolve inconsistency
						...
						if inconsistency_check:
							resolve_inconsistency_edge(interpretations_edge[t], comp, (label_key, bnd), ipl, t, fp_cnt, idx, atom_trace, rule_trace_edge, rule_trace_edge_atoms, rules_to_be_applied_edge_trace, facts_to_be_applied_edge_trace, store_interpretation_changes, mode='rule')
						else:
							u, changes = _update_edge(interpretations_edge[t], predicate_map_edge, comp, (label_key, bnd), ipl, rule_trace_edge, fp_cnt, t, set_static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_edge_trace, idx, facts_to_be_applied_edge_trace, rule_trace_edge_atoms, store_interpretation_changes, mode='rule', override=True)
	...
	def add_edge(self, edge, label_key):
		# This function is useful for pyreason gym, called externally
		_add_edge(edge[0], edge[1], self.neighbors, self.reverse_neighbors, self.nodes, self.edges, label_key, self.interpretations_node, self.interpretations_edge, self.predicate_map_edge, -1)
	...
	def add_node(self, node, labels):
		...
		if node not in self.nodes:
			...
			for label_key in labels:
				self.interpretations_node[node].world[label.Label(label_key)] = interval.closed(0, 1)
	...
	def get_dict(self):
		...
		for change in self.rule_trace_node:
			time, _, node, label_key, bnd = change
			interpretations[time][node][label_key._value] = (bnd.lower, bnd.upper)

			# If persistent, update all following timesteps as well
			if self. persistent:
				for t in range(time+1, self.time+1):
					interpretations[t][node][label_key._value] = (bnd.lower, bnd.upper)

		# Update interpretation edges
		for change in self.rule_trace_edge:
			time, _, edge, label_key, bnd, = change
			interpretations[time][edge][label_key._value] = (bnd.lower, bnd.upper)

			# If persistent, update all following timesteps as well
			if self. persistent:
				for t in range(time+1, self.time+1):
					interpretations[t][edge][label_key._value] = (bnd.lower, bnd.upper)
	...
	def get_final_num_ground_atoms(self):
		...
		for node in self.nodes:
			for label_key in self.interpretations_node[node].world:
				ga_cnt += 1
		for edge in self.edges:
			for label_key in self.interpretations_edge[edge].world:
				ga_cnt += 1
...
@numba.njit(cache=True)
def _ground_rule(rule, interpretations_node, interpretations_edge, predicate_map_node, predicate_map_edge, nodes, edges, neighbors, reverse_neighbors, atom_trace, allow_ground_rules, t):
	# Extract rule params
		...
		if rule_type == 'node':
			...
			for valid_e in valid_edge_groundings:
				...
				if add_head_edge_to_graph and (head_var_1, head_var_2) == (head_var_1_grounding, head_var_2_grounding):
					_add_edge(head_var_1, head_var_2, neighbors, reverse_neighbors, nodes, edges, label.Label(''), interpretations_node, interpretations_edge, predicate_map_edge, t)
...
@numba.njit(cache=True)
def get_rule_node_clause_grounding(clause_var_1, groundings, predicate_map, label_key, nodes):
	# The groundings for a node clause can be either a previous grounding or all possible nodes
	if label_key in predicate_map:
		grounding = predicate_map[label_key] if clause_var_1 not in groundings else groundings[clause_var_1]
	else:
...
@numba.njit(cache=True)
def get_rule_edge_clause_grounding(clause_var_1, clause_var_2, groundings, groundings_edges, neighbors, reverse_neighbors, predicate_map, label_key, edges):
	...
	if clause_var_1 not in groundings and clause_var_2 not in groundings:
		if label_key in predicate_map:
			edge_groundings = predicate_map[label_key]
		else:
...
def _update_node(interpretations, predicate_map, comp, na, ipl, rule_trace, fp_cnt, t_cnt, static, convergence_mode, atom_trace, save_graph_attributes_to_rule_trace, rules_to_be_applied_trace, idx, facts_to_be_applied_trace, rule_trace_atoms, store_interpretation_changes, mode, override=False):
	...
	if label_key not in world.world:
		world.world[label_key] = interval.closed(0, 1)
		if label_key in predicate_map:
			predicate_map[label_key].append(comp)
		else:
			predicate_map[label_key] = numba.typed.List([comp])

	# Check if update is necessary with previous bnd
	prev_bnd = world.world[label_key].copy()

	# override will not check for inconsistencies
	if override:
		world.world[label_key].set_lower_upper(bnd.lower, bnd.upper)
	else:
		world.update(label_key, bnd)
	world.world[label_key].set_static(static)
	if world.world[label_key]!=prev_bnd:
		updated = True
		updated_bnds.append(world.world[label_key])
		...
		if (save_graph_attributes_to_rule_trace or not mode=='graph-attribute-fact') and store_interpretation_changes:
			rule_trace.append((numba.types.uint16(t_cnt), numba.types.uint16(fp_cnt), comp, label_key, world.world[label_key].copy()))
			if atom_trace:
	...
	if updated:
		...
		for p1, p2 in ipl:
			if p1 == label_key:
				...
				if atom_trace:
					_update_rule_trace(rule_trace_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), world.world[p2], f'IPL: {label_key.get_value()}')
			...
			if p2 == label_key:
				...
				if atom_trace:
					_update_rule_trace(rule_trace_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), world.world[p1], f'IPL: {label_key.get_value()}')
	...
	if updated:
		...
		current_bnd = world.world[label_key]
		prev_t_bnd = interval.closed(world.world[label_key].prev_lower, world.world[label_key].prev_upper)
...
def are_satisfied_node(interpretations, comp, nas):
	result = True
	for (label_key, bnd) in nas:
		result = result and is_satisfied_node(interpretations, comp, (label_key, bnd))
	return result
...
def is_satisfied_node_comparison(interpretations, comp, na):
	...
	number = 0
	label_key, bnd = na
	l_str = label_key.value

	if not (label_key is None or bnd is None):
		# This is to prevent a key error in case the label is a specific label
		try:
			world = interpretations[comp]
...
def are_satisfied_edge(interpretations, comp, nas):
	result = True
	for (label_key, bnd) in nas:
		result = result and is_satisfied_edge(interpretations, comp, (label_key, bnd))
	return result
...
@numba.njit(cache=True)
def _add_node_to_interpretation(node, interpretations_node):
	interpretations_node[node] = world.World(numba.typed.List.empty_list(label.label_type))


@numba.njit(cache=True)
def _add_edge_to_interpretation(edge, interpretations_edge):
	interpretations_edge[edge] = world.World(numba.typed.List.empty_list(label.label_type))


@numba.njit(cache=True)
def _add_edge(source, target, neighbors, reverse_neighbors, nodes, edges, label_key, interpretations_node, interpretations_edge, predicate_map, t):
	...
	# Make sure, if label_key=='', not to add the label
	# Make sure, if edge exists, that we don't override the label_key label if it exists
	edge = (source, target)
	new_edge = False
	if edge not in edges:
		...
		if label_key.value!='':
			if edge not in interpretations_edge:
				interpretations_edge[edge] = world.World(numba.typed.List([label_key]))
			if label_key in predicate_map:
				predicate_map[label_key].append(edge)
			else:
				predicate_map[label_key] = numba.typed.List([edge])
		else:
			interpretations_edge[edge] = world.World(numba.typed.List.empty_list(label.label_type))
	else:
		if label_key not in interpretations_edge[edge].world and label_key.value!='':
			new_edge = True
			interpretations_edge[edge].world[label_key] = interval.closed(0, 1)
...
@numba.njit(cache=True)
def _add_edges(sources, targets, neighbors, reverse_neighbors, nodes, edges, label_key, interpretations_node, interpretations_edge, predicate_map, t):
	changes = 0
	edges_added = numba.typed.List.empty_list(edge_type)
	for source in sources:
		for target in targets:
			edge, new_edge = _add_edge(source, target, neighbors, reverse_neighbors, nodes, edges, label_key, interpretations_node, interpretations_edge, predicate_map, t)
...
def _delete_edge(edge, neighbors, reverse_neighbors, edges, interpretations_edge, predicate_map):
	...
	for label_key in predicate_map:
		if edge in predicate_map[label_key]:
			predicate_map[label_key].remove(edge)
...
def _delete_node(node, neighbors, reverse_neighbors, nodes, interpretations_node, predicate_map):
	...
	for label_key in predicate_map:
		if node in predicate_map[label_key]:
			predicate_map[label_key].remove(node)
